(function () {
  ...
  function pruneStoredIds(key) {
    var validIds = allCards.map(function (card) {
      return String(card.id);
    });

    var stored = getStored(key);

    var pruned = stored.filter(function (id) {
      return validIds.indexOf(id) !== -1;
    });

    if (pruned.length !== stored.length) {
      setStored(key, pruned);
    }
  }

  function pruneStaleStorage() {
    pruneStoredIds(READ_KEY);
    pruneStoredIds(SAVE_KEY);
  }
  ...
  function renderFeed() {
    ...
    feed.innerHTML = visible
      .map(function (card) {
        ...
        var hasSourceUrl = isValidHttpUrl(
          card.sourceUrl
        );
  ...
  function openReader(id) {
    ...
    if (source) {
      var readerHasSourceUrl = isValidHttpUrl(
        card.sourceUrl
      );
  ...
  function loadData() {
    fetch(DATA_URL, {
      ...
      .then(function (data) {
        ...
        // Open the article directly if the URL already points to one
        // (e.g. a shared link like #card-4), regardless of the current
        // category filter or how many cards are currently rendered.
        openReaderFromHash();
      })
  ...
  // Breaking-bar / sidebar links point to "#card-ID" anchors. If that
  // card isn't in the currently filtered/visible feed, the browser has
  // nothing to scroll to. Opening the reader on hashchange makes those
  // links (and any shared/copied link) work regardless of filter state.
  window.addEventListener(
    "hashchange",
    openReaderFromHash
  );
